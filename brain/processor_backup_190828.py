...
class ImageProcessor(threading.Thread):
    ...
    def _gmm_remove_background(self):
        ...
        for idx in range(self.exp.start_img, self.exp.end_img):
            ...
            y_bg = y[bg_retain, :]
            retain = random.choice(y_bg.shape[0], 50000, replace=False)
            y_bg = y_bg[retain, :]
    ...
    def _label_seeds(self):
        ...
        for idx in range(0, len(self.all_masks)):
            ...
            c = str(idx)
            filename = "seeds_labelled" + c + ".png"
            fig.savefig(pj(self.exp_images_dir, filename))
            plt.close(fig)
            self.all_rprops.append(self.panel_l_rprops)
            with open(l_rprops_f, "wb") as fh:
                pickle.dump(self.panel_l_rprops, fh)
    ...
    def _perform_classification(self):
        ...
        for panel_idx, panel_object in enumerate(self.panel_list):
            try:
                ...
                # panel_labels, panel_regionprops = self.all_rprops[panel_idx]
                J = [item[panel_idx] for item in self.all_rprops]
        ...
        if self.running:
            ...
            try:
                ...
                if self.exp.bg_remover == 'UNet':
                    ...
                    self._train_clfs([("sgd", SGDClassifier(n_jobs=-1, random_state=0, max_iter=1))])
                    # self.app.status_string.set("Removing background")
                    ...
                    self._train_clfs([("randf", RandomForestClassifier(n_estimators=50, n_jobs=-1, random_state=0))])
                    # self.app.status_string.set("Removing background")
                    self._remove_background()
                else:
